...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        operands = rest.map(scheme_eval_helper)
...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        if self.parent is not None and self.parent.lookup(symbol) is not None:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('arguments and parameters don''t match.')
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('Type error in some builtin procedures.')
        # END PROBLEM 3
...
def do_define_macro(expressions, env):
    ...
    macro_proc = macro_proc.apply_macro(formals, env)
