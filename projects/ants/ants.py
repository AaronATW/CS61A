...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while place is not beehive:
            if place.bees and throw_distance >= self.min_range and throw_distance <= self.max_range:
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt does nothing. It has a lot of armor."""
    name = 'Wall'
    food_cost = 4
    implemented = True
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower is a throwerant which can live in water."""
    name = 'Scuba'
    food_cost = 6
    implemented = True
    is_watersafe = True
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        if self.real:
            ...
            while behind_place:
                if behind_place.ant:
                    if isinstance(behind_place.ant, ContainerAnt):
                        if behind_place.ant.contained_ant and not behind_place.ant.contained_ant.doubled:
                            behind_place.ant.contained_ant.damage = behind_place.ant.contained_ant.damage * 2
                            behind_place.ant.contained_ant.doubled = True
                    if not behind_place.ant.doubled:
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC
        assert isinstance(target, Bee), "The target is not a bee!"
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        # BEGIN Problem OPTIONAL
        result = {}
        current_place, current_distance = self.place, 0
        while current_place is not beehive:
            if current_place.ant:
                result[current_place.ant] = current_distance
                if isinstance(current_place.ant, ContainerAnt) and current_place.ant.contained_ant:
                    result[current_place.ant.contained_ant] = current_distance
            for bee in current_place.bees:
                result[bee] = current_distance
            current_place, current_distance = current_place.entrance, current_distance + 1
        del result[self]
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem OPTIONAL
        result = self.damage - distance * 0.2 - self.insects_shot * 0.05
        if result < 0:
            result = 0
        return result
        # END Problem OPTIONAL
