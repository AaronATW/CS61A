...
def multiple_references_explanation():
    ...
    Because the calulation between intervals doesn't obey the rules used on numbers. 
    The two formulas are not equivalent at all."""
...
def quadratic(x, a, b, c):
    ...
    y1, y2 = f(upper_bound(x)), f(lower_bound(x))
    if a == 0:
        return interval(min(y1, y2), max(y1, y2))
    peak_x = -b / (2 * a)
    y0 = f(peak_x)
    if upper_bound(x) < peak_x or lower_bound(x) > peak_x:
        return interval(min(y1, y2), max(y1, y2))
    elif a > 0:
        return interval(y0, max(y1, y2))
    else:
        return interval(min(y1, y2), y0)
