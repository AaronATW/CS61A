...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, password_input):
        nonlocal balance
        if len(past_wrong_password) >= 3:
            return "Too many incorrect attempts. Attempts: ['" + past_wrong_password[0] + "', '" + past_wrong_password[1] + "', '" + past_wrong_password[2] + "']"
            ...
            if amount <= balance:
                ...
                return 'Insufficient funds'
        elif len(past_wrong_password) < 3:
            past_wrong_password.append(password_input)
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    def joint(amount, password_input):
        if password_input == old_pass or password_input == new_pass:
...
def remainders_generator(m):
    ...
    11
    """
    def each_remainder_generator(n):
        naturals_generator = naturals()
        while True:
            number = next(naturals_generator)
            if number % m == n:
                yield number
    for n in range(m):
        yield each_remainder_generator(n)
