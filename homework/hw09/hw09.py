...
def interleaved_sum(n, odd_term, even_term):
    ...
    True
    """
    def interleaved_sum_helper(odd_or_even, current_n):
        if current_n > n:
            return 0
        if odd_or_even == 'odd':
            return odd_term(current_n) + interleaved_sum_helper('even', current_n + 1)
        else:
            return even_term(current_n) + interleaved_sum_helper('odd', current_n + 1)
    return interleaved_sum_helper('odd', 1)
...
def mutate_reverse(link):
    ...
    """
    def link_append(link, label):
        if link.rest is Link.empty:
            link.rest = Link(label)
        else:
            link_append(link.rest, label)

    if link.rest is Link.empty:
        return
    mutate_reverse(link.rest)
    link_append(link.rest, link.first)
