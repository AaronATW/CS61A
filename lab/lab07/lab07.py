...
def subseqs(s):
    ...
    if len(s) == 0:
        ...
        suffix = subseqs(s[1:])
        return suffix + insert_into_all(s[0], suffix)
...
def reverse(lst):
    ...
    receiver = []
    while lst:
        receiver.append(lst.pop())
    while receiver:
        lst.append(receiver.pop(0))
...
def make_glookup(class_assignments):
    ...
    def glookup(assignment, score):
        nonlocal total_score, current_score
...
def make_advanced_counter_maker():
    ...
    def make_counter():
        individual_count = 0
        def counter(command):
            nonlocal global_count, individual_count
            if command == "count":
                individual_count += 1
