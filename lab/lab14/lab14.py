def prune_min(t):
    ...
    if left_branch.label < right_branch.label:
        t.branches = [left_branch]
        prune_min(left_branch)
    else:
        t.branches = [right_branch]
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        match, cost = '+[' + code_char + ']' + match, cost + 1
        possibilities.append((match, cost))
        # Delete
        match, cost = helper_align(skeleton_idx + 1, code_idx)
        match, cost = '-[' + skel_char + ']' + match, cost + 1
        possibilities.append((match, cost))
...
def num_splits(s, d):
    ...
    12
    """
    def split_helper(s, sum):
        if len(s) == 0:
            if -d <= sum <= d:
                return 1
            else:
                return 0
        return split_helper(s[1:], sum + s[0]) + split_helper(s[1:], sum - s[0])
    return split_helper(s[1:], s[0])
